...
import (
	...
	"github.com/strangelove-ventures/cosmos-client/client/chain_registry"
	"go.uber.org/zap"
)
...
func cmdChainsRegistryList(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return a.GetDefaultClient().PrintObject(chains)
		},
	}
	return cmd
}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, chain := range args {
				...
				chainConfig, err := client.GetChainConfigWithOpts(cmd.Context(), chainInfo, nil)
				if err != nil {
					a.Log.Info(
				...
				a.Config.clientConfig.SetChainConfig(chain, chainConfig)
			}
			if overwriteConfig {
...
func cmdChainsDelete(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:    cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			originalChainCount := len(a.Config.clientConfig.GetChainConfigs())
			for _, arg := range args {
				if a.Config.clientConfig.GetDefaultChain() == arg {
					fmt.Fprintf(cmd.ErrOrStderr(), "Ignoring delete request for %s, unable to delete default chain.\n", arg)
					continue
				}
				delete(a.Config.clientConfig.GetChainConfigs(), arg)
			}

			// If nothing was removed, there's no need to update the configuration file.
			if len(a.Config.clientConfig.GetChainConfigs()) == originalChainCount {
				return nil
			}
...
func cmdChainsEdit(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			var conf *client.ChainClientConfig
			var ok bool

			if conf, ok = a.Config.clientConfig.GetChainConfigs()[args[0]]; !ok {
			...
			case "rpc-addr":
				conf.RPCAddr = args[2]
			case "grpc-addr":
				conf.GRPCAddr = args[2]
			case "account-prefix":
				conf.AccountPrefix = args[2]
			case "gas-adjustment":
				fl, err := strconv.ParseFloat(args[2], 64)
				if err != nil {
					return err
				}
				conf.GasAdjustment = fl
			case "gas-prices":
				conf.GasPrices = args[2]
			case "min-gas-amount":
				ga, err := strconv.ParseUint(args[2], 10, 64)
				if err != nil {
					return err
				}
				conf.MinGasAmount = ga
			case "debug":
...
func cmdChainsList(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			return a.GetDefaultClient().PrintObject(a.Config.clientConfig.GetChainConfigs())
		},
	}
	return cmd
}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				...
				names := make([]string, 0, len(a.Config.clientConfig.GetChainConfigs()))
				for name := range a.Config.clientConfig.GetChainConfigs() {
			...
			if ch, ok := a.GetChainConfigs()[args[0]]; ok {
				return a.GetDefaultClient().PrintObject(ch)
			}
...
func cmdChainsSetDefault(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if _, ok := a.GetChainConfigs()[args[0]]; ok {
				a.Config.clientConfig.SetDefaultChain(args[0])
				return a.OverwriteConfig(a.Config)
			}
			return fmt.Errorf("chain %s not found", args[0])
		},
...
func cmdChainsShowDefault(a *appState) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			fmt.Fprintln(cmd.OutOrStdout(), a.Config.clientConfig.GetDefaultChain())
			return nil
		},
	}
	return cmd
