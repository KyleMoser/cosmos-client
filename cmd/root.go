...
func NewRootCmd(log *zap.Logger, atom zap.AtomicLevel, o map[string]ClientOverrides) *cobra.Command {
	...
	// This is similar to Strangelove's Lens tool, we changed the home directory to avoid conflicts.
	defaultHome := os.ExpandEnv("$HOME/.chainclient")

	// RootCmd represents the base command when called without any subcommands
	var rootCmd = &cobra.Command{
		Use:   appName,
		Short: "Basic CLI functionality for Cosmos blockchains.",
	}
	...
	if extraCommands != nil {
		rootCmd.AddCommand(extraCommands...)
	}
